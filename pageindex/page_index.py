...
def generate_toc_continue(toc_content, part, model="gpt-4o-2024-11-20"):
    ...
    继续生成目录结构
    """
    prompt = f"""你是一个专业的文档结构分析专家。我已经有了一部分文档的目录结构，请基于现有结构，为新的文档内容生成相应的目录结构。
    ...
    [
        {{
            "title": "章节标题",
            "physical_index": 页码数字,
            "appear_start": "yes/no" (标题是否在该页开头出现)
        }},
        ...
    ]

    注意：
    1. 只返回JSON格式的结果，不要包含其他说明文字
    2. physical_index必须是数字
    3. appear_start必须是"yes"或"no"
    4. 确保JSON格式正确，可以被解析
    5. 新生成的结构应该与现有结构保持一致的风格
    """
    ...
    try:
        ...
        logging.error(f"生成目录结构时发生错误: {str(e)}")
        return toc_content
...
def generate_toc_init(part, model=None):
    ...
    生成初始目录结构
    """
    prompt = f"""你是一个专业的文档结构分析专家。请仔细阅读以下文档内容，并生成一个包含标题和页码的结构化目录。
